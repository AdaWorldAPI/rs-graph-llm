...
//! unit-pass), and that the fail-closed cite-gate (now using the contract's
//! `SourceSpan::is_cited`) halts a flow with uncited claims before it can commit.
//! `commit` is a terminal node (`NextAction::End`) standing in for promote/persist.
